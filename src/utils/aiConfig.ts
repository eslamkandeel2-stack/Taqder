...
export const SUPPORTED_AI_MODELS: AIModelOption[] = [
  ...
  },
  {
    ...
    description: 'نموذج خفيف جداً يستهلك أقل قدر من الموارد، مناسب للأجهزة الضعيفة والاتصالات البطيئة.',
  },
  {
    id: 'gemini-3.1-pro-preview',
    name: 'Gemini 3.1 Pro Preview',
    badge: 'فائق الذكاء والبلاغة 🧠',
    description: 'أقوى النماذج للبلاغة العربية المعقدة ونظم الأبيات الشعرية التكريمية الموزونة والاستشارات العميقة.',
  },
];
...
export async function testAIConnection(settings?: AISettings): Promise<{
  ...
  try {
    const endpoint = (cfg.customApiUrl?.trim() || '') + '/api/test-ai-connection';
    const response = await fetch(endpoint, {
      method: 'POST',
      headers: getAIRequestHeaders(cfg),
      body: JSON.stringify({
        apiKey: cfg.apiKey?.trim() || undefined,
        model: cfg.model?.trim() || 'gemini-3.7-flash',
      }),
    });
...
// Highly sophisticated local phrase generator when offline or API is not reachable
export function generateLocalTextVariations(params: ImproveTextParams): TextVariation[] {
  ...
  if (params.type === 'title') {
    return [
      { id: 1, text: isFemale ? 'شهادة شكر وتقدير ووفاء' : 'شهادة شكر وتقدير ووفاء', styleLabel: 'كلاسيكي رسمي فخم' },
      { id: 2, text: isFemale ? 'وسام التميز والتفوق الأكاديمي' : 'وسام التميز والتفوق الأكاديمي', styleLabel: 'أكاديمي ملكي' },
      { id: 3, text: isFemale ? 'شهادة إشادة وإنجاز استثنائي' : 'شهادة إشادة وإنجاز استثنائي', styleLabel: 'حماسي ملهم' },
    ];
  }
...
export async function improveCertificateTextWithAi(params: ImproveTextParams): Promise<{
  ...
  try {
    const endpoint = (cfg.customApiUrl?.trim() || '') + '/api/ai-improve-text';
    const response = await fetch(endpoint, {
      method: 'POST',
      headers: getAIRequestHeaders(cfg),
      body: JSON.stringify({
  ...
  // Gracefully fallback to high quality local variations
